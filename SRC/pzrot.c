...
*/
   if( *n == 1 )
   {
      ...
      else if( ( myrow == iyrow ) && ( mycol == iycol ) )
      {
         ...
         zrot_( n, xwork, n, YPTR( iiy, jjy ), n, c, s );
      }
      return;
   }
   ...
   {               /* X and Y are both distributed over a process row */
      ...
      if( ixrow == iyrow )
      {
         if( myrow == ixrow )
         {
            zrot_( &nq, XPTR( iix, jjx ), &desc_X[LLD_],
      ...
      else
      {
         if( myrow == ixrow )
         {
            ...
            if( !ScaLAPACK_Index64ToSizeT( (ScaLAPACK_Index64) nq, &alloc_count ) ||
               ...
               PB_Cabort( ictxt, "PZROT", -1 );
            buff = (complex16 *)getpbbuf64( "PZROT", alloc_bytes );
