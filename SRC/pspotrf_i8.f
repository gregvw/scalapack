      SUBROUTINE PSPOTRF_I8( UPLO, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
*     INTEGER*8 native version of PSPOTRF.
*     All public integer arguments (N, IA, JA, DESCA) are INTEGER*8;
*     INFO stays INTEGER.
*     Uses I8 calls for PSSYRK and PSTRSM; PSPOTF2 stays legacy
...
*  =======
*
*  PSPOTRF_I8 computes the Cholesky factorization of an N-by-N real
...
*     .. External Subroutines ..
     ...
     $                   PB_TOPGET, PB_TOPSET, PCHK1MAT_I8, PSPOTF2,
     $                   PSSYRK_I8, PSTRSM_I8, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPOTRF_I8', -INFO )
         RETURN
      END IF
*
...
*
     ...
     $   RETURN
*
*     Range check: PSPOTF2 requires narrowing to INTEGER.
*     N, IA, JA must fit in INTEGER for the POTF2 calls.
*     (The block size is always small, so JB is fine.)
*
      IF( N.GT.INTMAX .OR. IA.GT.INTMAX .OR. JA.GT.INTMAX ) THEN
         INFO = -1
         CALL PXERBLA( ICTXT, 'PSPOTRF_I8', 1 )
         RETURN
      END IF
*
*     Narrow descriptor for PSPOTF2 calls (legacy INTEGER interface)
*
...
*
         CALL PSPOTF2( UPLO, JB, A, INT( IA ), INT( JA ), DESCA4,
     $                 INFO )
         IF( INFO.NE.0 )
     $      GO TO 30
*
...
*
            CALL PSTRSM_I8( 'Left', UPLO, 'Transpose', 'Non-Unit',
     $                      INT( JB, 8 ), N-JB, ONE, A, IA, JA, DESCA,
     $                      A, IA, JA+JB, DESCA )
*
*           Update the trailing matrix, A = A - U'*U
*
            CALL PSSYRK_I8( UPLO, 'Transpose', N-JB, INT( JB, 8 ),
     $                      -ONE, A, IA, JA+JB, DESCA, ONE, A, IA+JB,
     $                      JA+JB, DESCA )
         END IF
*
...
*           Perform unblocked Cholesky factorization on JB block
*
            CALL PSPOTF2( UPLO, JB, A, INT( I ), INT( J ), DESCA4,
     $                    INFO )
            IF( INFO.NE.0 ) THEN
               INFO = INFO + INT( J - JA )
               GO TO 30
...
*
               CALL PSTRSM_I8( 'Left', UPLO, 'Transpose', 'Non-Unit',
     $                         INT( JB, 8 ), N-J-JB+JA, ONE, A, I, J,
     $                         DESCA, A, I, J+JB, DESCA )
*
*              Update the trailing matrix, A = A - U'*U
*
               CALL PSSYRK_I8( UPLO, 'Transpose', N-J-JB+JA,
...
*
            CALL PSTRSM_I8( 'Right', UPLO, 'Transpose', 'Non-Unit',
     $                      N-JB, INT( JB, 8 ), ONE, A, IA, JA, DESCA,
     $                      A, IA+JB, JA, DESCA )
*
*           Update the trailing matrix, A = A - L*L'
*
            CALL PSSYRK_I8( UPLO, 'No Transpose', N-JB, INT( JB, 8 ),
...
*
               CALL PSTRSM_I8( 'Right', UPLO, 'Transpose', 'Non-Unit',
     $                         N-J-JB+JA, INT( JB, 8 ), ONE, A, I, J,
     $                         DESCA, A, I+JB, J, DESCA )
*
*              Update the trailing matrix, A = A - L*L'
*
               CALL PSSYRK_I8( UPLO, 'No Transpose', N-J-JB+JA,
