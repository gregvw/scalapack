      SUBROUTINE PDLATRD_I8( UPLO, N, NB, A, IA, JA, DESCA, D, E,
...
*     .. External Subroutines ..
     ...
     $                   DGEBS2D, INFOG2L_I8,
     $                   PDAXPY_I8, PDDOT_I8, PDELGET_I8,
     $                   PDELSET_I8, PDGEMV_I8, PDLARFG_I8,
     $                   PDSCAL_I8, PDSYMV_I8
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*
            CALL PDGEMV_I8( 'No transpose', K8, N-K8, -ONE, A, IA,
     $                      J8+1, DESCA, W, IW+K8-1, JW+KW8, DESCW,
     $                      DESCW( M_ ), ONE, A, IA, J8, DESCA, 1_8 )
            CALL PDGEMV_I8( 'No transpose', K8, N-K8, -ONE, W, IW,
     $                      JW+KW8, DESCW, A, I8, J8+1, DESCA,
     $                      DESCA( M_ ), ONE, A, IA, J8, DESCA, 1_8 )
            IF( N-K8.GT.0 )
     $         CALL PDELSET_I8( A, I8, J8+1, DESCA, E( JP8 ) )
*
            JP8 = MIN( JJ8+KW8-1, NQ8 )
            CALL PDLARFG_I8( K8-1, E( JP8 ), I8-1, J8, A, IA, J8,
     $                       DESCA, 1_8, TAU )
            CALL PDELSET_I8( A, I8-1, J8, DESCA, ONE )
*
            CALL PDSYMV_I8( 'Upper', K8-1, ONE, A, IA, JA, DESCA,
     $                      A, IA, J8, DESCA, 1_8, ZERO, W, IW,
     $                      JW+KW8-1, DESCW, 1_8 )
*
            JWK8 = MOD( K8-1, DESCWK( NB_ ) ) + 2
            CALL PDGEMV_I8( 'Transpose', K8-1, N-K8, ONE, W, IW,
     $                      JW+KW8, DESCW, A, IA, J8, DESCA, 1_8,
     $                      ZERO, WORK, 1_8, JWK8, DESCWK,
     $                      DESCWK( M_ ) )
            CALL PDGEMV_I8( 'No transpose', K8-1, N-K8, -ONE, A, IA,
     $                      J8+1, DESCA, WORK, 1_8, JWK8, DESCWK,
     $                      DESCWK( M_ ), ONE, W, IW, JW+KW8-1,
     $                      DESCW, 1_8 )
            CALL PDGEMV_I8( 'Transpose', K8-1, N-K8, ONE, A, IA,
     $                      J8+1, DESCA, A, IA, J8, DESCA, 1_8,
     $                      ZERO, WORK, 1_8, JWK8, DESCWK,
     $                      DESCWK( M_ ) )
            CALL PDGEMV_I8( 'No transpose', K8-1, N-K8, -ONE, W, IW,
     $                      JW+KW8, DESCW, WORK, 1_8, JWK8, DESCWK,
     $                      DESCWK( M_ ), ONE, W, IW, JW+KW8-1,
     $                      DESCW, 1_8 )
            CALL PDSCAL_I8( K8-1, TAU( JP8 ), W, IW, JW+KW8-1,
     $                      DESCW, 1_8 )
*
            CALL PDDOT_I8( K8-1, ALPHA, W, IW, JW+KW8-1, DESCW,
     $                     1_8, A, IA, J8, DESCA, 1_8 )
            IF( MYCOL.EQ.IACOL )
     $         ALPHA = -HALF*TAU( JP8 )*ALPHA
            CALL PDAXPY_I8( K8-1, ALPHA, A, IA, J8, DESCA, 1_8,
     $                      W, IW, JW+KW8-1, DESCW, 1_8 )
            IF( MYCOL.EQ.IACOL ) THEN
               CALL PDELGET_I8( 'E', ' ', D( JP8 ), A, I8, J8,
     $                          DESCA )
            END IF
*
   10    CONTINUE
...
*
         DO 20 J8 = JA, JA+NB-1
            ...
            K8 = J8 - JA + 1
*
            CALL PDGEMV_I8( 'No transpose', N-K8+1, K8-1, -ONE, A,
     $                      I8, JA, DESCA, W, IW+K8-1, JW, DESCW,
     $                      DESCW( M_ ), ONE, A, I8, J8, DESCA,
     $                      1_8 )
            CALL PDGEMV_I8( 'No transpose', N-K8+1, K8-1, -ONE, W,
...
*
            ...
            CALL PDLARFG_I8( N-K8, E( JP8 ), I8+1, J8, A, I8+2,
     $                       J8, DESCA, 1_8, TAU )
            CALL PDELSET_I8( A, I8+1, J8, DESCA, ONE )
*
            CALL PDSYMV_I8( 'Lower', N-K8, ONE, A, I8+1, J8+1,
     $                      DESCA, A, I8+1, J8, DESCA, 1_8, ZERO,
     $                      W, IW+K8, JW+K8-1, DESCW, 1_8 )
*
            CALL PDGEMV_I8( 'Transpose', N-K8, K8-1, ONE, W, IW+K8,
     $                      JW, DESCW, A, I8+1, J8, DESCA, 1_8,
     $                      ZERO, WORK, 1_8, 1_8, DESCWK,
     $                      DESCWK( M_ ) )
            CALL PDGEMV_I8( 'No transpose', N-K8, K8-1, -ONE, A,
     $                      I8+1, JA, DESCA, WORK, 1_8, 1_8, DESCWK,
     $                      DESCWK( M_ ), ONE, W, IW+K8, JW+K8-1,
     $                      DESCW, 1_8 )
            CALL PDGEMV_I8( 'Transpose', N-K8, K8-1, ONE, A, I8+1,
     $                      JA, DESCA, A, I8+1, J8, DESCA, 1_8,
     $                      ZERO, WORK, 1_8, 1_8, DESCWK,
     $                      DESCWK( M_ ) )
            CALL PDGEMV_I8( 'No transpose', N-K8, K8-1, -ONE, W,
     $                      IW+K8, JW, DESCW, WORK, 1_8, 1_8,
     $                      DESCWK, DESCWK( M_ ), ONE, W, IW+K8,
     $                      JW+K8-1, DESCW, 1_8 )
            CALL PDSCAL_I8( N-K8, TAU( JP8 ), W, IW+K8, JW+K8-1,
     $                      DESCW, 1_8 )
            CALL PDDOT_I8( N-K8, ALPHA, W, IW+K8, JW+K8-1, DESCW,
     $                     1_8, A, I8+1, J8, DESCA, 1_8 )
            IF( MYCOL.EQ.IACOL )
     $         ALPHA = -HALF*TAU( JP8 )*ALPHA
            CALL PDAXPY_I8( N-K8, ALPHA, A, I8+1, J8, DESCA, 1_8,
     $                      W, IW+K8, JW+K8-1, DESCW, 1_8 )
            IF( MYCOL.EQ.IACOL ) THEN
               CALL PDELGET_I8( 'E', ' ', D( JP8 ), A, I8, J8,
     $                          DESCA )
