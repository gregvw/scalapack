      SUBROUTINE PZHENTRD_I8( UPLO, N, A, IA, JA, DESCA, D, E, TAU,
     $                       WORK, LWORK, RWORK, LRWORK, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
*     INTEGER*8 bridge version of PZHENTRD.
*
...
*     .. External Subroutines ..
     ...
     $                   CHK1MAT_I8, DESCSET_I8, ZHETRD,
     $                   DGAMN2D, PCHK1MAT_I8, PZELSET_I8,
     $                   PZLAMR1D_I8, PDLAMR1D_I8,
     $                   PZLATRD_I8, PZHER2K_I8, PZHETD2_I8, PZHETTRD,
     $                   PZTRMR2D_I8, PDTRMR2D_I8,
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZHENTRD_I8', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
     ...
     $                    NPS8 )
*
         CALL PZTRMR2D_I8( UPLO, 'N', N, N, A, IA, JA, DESCA,
...
*
     ...
     $          LLWORK.GT.INTMAX ) THEN
               CALL PXERBLA( ICTXT, 'PZHENTRD_I8', -2 )
...
*
            IF( NPROWB.EQ.1 ) THEN
               CALL ZHETRD( UPLO, N4, WORK( INDB ), NPS4,
     $                      RWORK( INDRD ), RWORK( INDRE ),
     $                      WORK( INDTAU ), WORK( INDW ),
     $                      INT( LLWORK ), IINFO )
            ELSE
               CALL PZHETTRD( 'L', N4, WORK( INDB ), 1, 1, DESCB4,
     $                        RWORK( INDRD ), RWORK( INDRE ),
     $                        WORK( INDTAU ), WORK( INDW ),
     $                        INT( LLWORK ), IINFO )
            END IF
         END IF
*
*        D, E are REAL — use PDLAMR1D_I8
*        TAU is COMPLEX — use PZLAMR1D_I8
*
         CALL PDLAMR1D_I8( N-1, RWORK( INDRE ), 1_8, 1_8, DESCB,
     $                     E, 1_8, JA, DESCA )
         CALL PDLAMR1D_I8( N, RWORK( INDRD ), 1_8, 1_8, DESCB,
     $                     D, 1_8, JA, DESCA )
         CALL PZLAMR1D_I8( N, WORK( INDTAU ), 1_8, 1_8, DESCB,
     $                     TAU, 1_8, JA, DESCA )
*
         CALL PZTRMR2D_I8( UPLO, 'N', N, N, WORK( INDB ), 1_8, 1_8,
     $                     DESCB, A, IA, JA, DESCA,
     $                     INT( ICTXT, 8 ) )
*
         IF( MYROWB.GE.0 )
...
*
               CALL PZLATRD_I8( UPLO, K8+JB-1, JB, A, IA,
     $              JA, DESCA, D, E, TAU, WORK, 1_8, 1_8,
     $              DESCW, WORK( IPW ) )
*
               CALL PZHER2K_I8( UPLO, 'No transpose', K8-1,
     $              INT( JB, 8 ), -CONE, A, IA, JA+K8-1, DESCA,
     $              WORK, 1_8, 1_8, DESCW, ONE, A, IA,
     $              JA, DESCA )
*
               JX8 = MIN( INDXG2L_I8( JA+K8-1, INT( NB, 8 ), 0,
     $              IACOL, NPCOL ), NQ )
               CALL PZELSET_I8( A, IA+K8-2, JA+K8-1, DESCA,
     $              DCMPLX( E( JX8 ) ) )
*
...
*
   10       CONTINUE
*
            CALL PZHETD2_I8( UPLO, MIN( N, INT( NB, 8 ) ), A, IA,
...
*
               CALL PZLATRD_I8( UPLO, N-K8+1, NB, A, IA+K8-1, JA+K8-1,
     $              DESCA, D, E, TAU, WORK, K8, 1_8, DESCW,
     $              WORK( IPW ) )
*
               CALL PZHER2K_I8( UPLO, 'No transpose', N-K8-NB+1,
     $              INT( NB, 8 ), -CONE, A, IA+K8-1+NB, JA+K8-1,
     $              DESCA, WORK, K8+NB, 1_8, DESCW, ONE,
     $              A, IA+K8-1+NB, JA+K8-1+NB, DESCA )
*
               JX8 = MIN( INDXG2L_I8( JA+K8+NB-2, INT( NB, 8 ), 0,
     $              IACOL, NPCOL ), NQ )
               CALL PZELSET_I8( A, IA+K8+NB-1, JA+K8+NB-2, DESCA,
     $              DCMPLX( E( JX8 ) ) )
*
...
*
   20       CONTINUE
*
            CALL PZHETD2_I8( UPLO, INT( KK, 8 ), A, IA+K8-1,
