      SUBROUTINE PSGETRF_I8( M, N, A, IA, JA, DESCA, IPIV, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
*     INTEGER*8 native version of PSGETRF.
*     All public integer arguments are INTEGER*8 except IPIV and INFO.
*     Uses I8 PBLAS calls (PSTRSM_I8, PSGEMM_I8, PSLASWP_I8).
*     PSGETF2 remains legacy (operates on a single block).
*
*     .. Scalar Arguments ..
      INTEGER*8          M, N, IA, JA
      INTEGER            INFO
...
*     .. External Subroutines ..
     ...
     $                   PSGEMM_I8, PSGETF2, PSLASWP_I8, PSTRSM_I8,
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSGETRF_I8', -INFO )
         RETURN
      END IF
*
...
*
     ...
     $    IA.GT.INTMAX .OR. JA.GT.INTMAX ) THEN
         INFO = -1
         CALL PXERBLA( ICTXT, 'PSGETRF_I8', 1 )
         RETURN
      END IF
*
...
*
     ...
     $          IA+M-1 )
      ...
      JB = INT( JN - JA + 1 )
*
      CALL PSGETF2( INT( M ), JB, A, INT( IA ), INT( JA ), DESCA4,
     $              IPIV, INFO )
*
      IF( JB+1.LE.N ) THEN
*
         CALL PSLASWP_I8( 'Forward', 'Rows', N-INT( JB, 8 ),
     $                    A, IA, JN+1, DESCA, IA, IN, IPIV )
*
         CALL PSTRSM_I8( 'Left', 'Lower', 'No transpose', 'Unit',
     $                   INT( JB, 8 ), N-INT( JB, 8 ), ONE,
     $                   A, IA, JA, DESCA, A, IA, JN+1, DESCA )
*
         IF( JB+1.LE.M ) THEN
*
            CALL PSGEMM_I8( 'No transpose', 'No transpose',
...
*
      DO 10 J = JN+1, JA+MN-1, NB8
         ...
         I = IA + J - JA
*
         CALL PSGETF2( INT( M-J+JA ), JB, A, INT( I ), INT( J ),
     $                 DESCA4, IPIV, IINFO )
*
         IF( INFO.EQ.0 .AND. IINFO.GT.0 )
     $      INFO = IINFO + INT( J - JA )
*
         CALL PSLASWP_I8( 'Forward', 'Rowwise', J-JA,
     $                    A, IA, JA, DESCA, I, I+INT( JB, 8 )-1,
     $                    IPIV )
*
         IF( J-JA+JB+1.LE.N ) THEN
*
            CALL PSLASWP_I8( 'Forward', 'Rowwise',
     $                       N-J-INT( JB, 8 )+JA,
