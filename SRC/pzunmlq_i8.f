      SUBROUTINE PZUNMLQ_I8( SIDE, TRANS, M, N, K, A, IA, JA, DESCA,
     $                       TAU, C, IC, JC, DESCC, WORK, LWORK,
     $                       INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
*     INTEGER*8 thin wrapper around PZUNMLQ.
*
*  Overwrites the general complex distributed M-by-N matrix sub( C )
*  with Q*sub(C) or Q**H*sub(C) or sub(C)*Q or sub(C)*Q**H.
*
*     .. Scalar Arguments ..
...
*     .. External Subroutines ..
      EXTERNAL           NARROW_DESC8, PZUNMLQ, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          INT
*     ..
...
*
     ...
     $    IC.GT.INTMAX .OR. JC.GT.INTMAX ) THEN
         INFO = -1
         CALL PXERBLA( ICTXT, 'PZUNMLQ_I8', -1 )
         RETURN
      END IF
*
      M4  = INT( M )
...
*
      CALL PZUNMLQ( SIDE, TRANS, M4, N4, K4, A, IA4, JA4, DESCA4,
