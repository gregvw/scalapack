      SUBROUTINE PDSYNTRD_I8( UPLO, N, A, IA, JA, DESCA, D, E, TAU,
     $                        WORK, LWORK, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*     .. External Subroutines ..
     ...
     $                   PDLAMR1D_I8, PDLATRD, PDSYR2K, PDSYTD2,
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PDSYNTRD_I8', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL PDTRMR2D_I8( UPLO, 'N', N, N, A, IA, JA, DESCA,
...
*
         CALL PDLAMR1D_I8( N-1, WORK( INDE ), 1_8, 1_8, DESCB,
     $                     E, 1_8, JA, DESCA )
         CALL PDLAMR1D_I8( N, WORK( INDD ), 1_8, 1_8, DESCB,
     $                     D, 1_8, JA, DESCA )
         CALL PDLAMR1D_I8( N, WORK( INDTAU ), 1_8, 1_8, DESCB,
     $                     TAU, 1_8, JA, DESCA )
*
         CALL PDTRMR2D_I8( UPLO, 'N', N, N, WORK( INDB ), 1_8, 1_8,
     $                     DESCB, A, IA, JA, DESCA,
     $                     INT( ICTXT, 8 ) )
*
         IF( MYROWB.GE.0 )
...
*
            DO 10 K8 = N - KK + 1, INT( NB, 8 ) + 1, -INT( NB, 8 )
               ...
               J4 = INT( JA + K8 - 1 )
*
               CALL PDLATRD( UPLO, K4+JB-1, JB, A, INT( IA ),
...
*
     ...
     $              IACOL, NPCOL ), NQ )
               CALL PDELSET_I8( A, IA+K8-2, JA+K8-1, DESCA,
     $              E( JX8 ) )
*
...
*
            DO 20 K8 = 1, N - NB, INT( NB, 8 )
               ...
               K4 = INT( K8 )
*
               CALL PDLATRD( UPLO, INT( N-K8+1 ), NB, A, I4, J4,
...
*
     ...
     $              IACOL, NPCOL ), NQ )
               CALL PDELSET_I8( A, IA+K8+NB-1, JA+K8+NB-2, DESCA,
     $              E( JX8 ) )
*
