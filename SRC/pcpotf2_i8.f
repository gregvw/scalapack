      SUBROUTINE PCPOTF2_I8( UPLO, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
*     INTEGER*8 native version of PCPOTF2.
*     All public integer arguments are INTEGER*8; INFO stays INTEGER.
*     Local BLAS calls (CGEMV, CSSCAL, CLACGV, CCDOTC) use INT()
*     narrowing for their integer arguments.
*
*     .. Scalar Arguments ..
      CHARACTER          UPLO
      INTEGER*8          N, IA, JA
...
*  =======
*
*  PCPOTF2_I8 computes the Cholesky factorization of a complex
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_ABORT, BLACS_GRIDINFO, CCDOTC,
     $                   CHK1MAT_I8, CGEMV, CLACGV, CSSCAL,
     $                   IGEBR2D, IGEBS2D, INFOG2L_I8, PB_TOPGET,
     $                   PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          INT, MOD, REAL, SQRT
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPOTF2_I8', -INFO )
...
*
                  CALL CCDOTC( INT( J-JA ), DOTC, A( IOFFA ), 1,
     $                         A( IOFFA ), 1 )
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + LDA
                     CALL CLACGV( INT( J-JA ), A( IOFFA ), 1 )
                     CALL CGEMV( 'Transpose', INT( J-JA ),
     $                           INT( JA+N-J-1 ), -CONE,
     $                           A( IOFFA+LDA ), INT( LDA ),
     $                           A( IOFFA ), 1,
     $                           CONE, A( ICURR ), INT( LDA ) )
                     CALL CLACGV( INT( J-JA ), A( IOFFA ), 1 )
                     CALL CSSCAL( INT( JA+N-J-1 ), ONE / AJJ,
     $                            A( ICURR ), INT( LDA ) )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + LDA
...
*
                  CALL CCDOTC( INT( J-JA ), DOTC, A( IOFFA ),
     $                         INT( LDA ), A( IOFFA ), INT( LDA ) )
                  AJJ = REAL( A( IDIAG ) - DOTC )
                  IF ( AJJ.LE.ZERO ) THEN
                     A( IDIAG ) = AJJ
                     INFO = INT( J - JA + 1 )
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + 1
                     CALL CLACGV( INT( J-JA ), A( IOFFA ),
     $                            INT( LDA ) )
                     CALL CGEMV( 'No transpose', INT( JA+N-J-1 ),
     $                           INT( J-JA ), -CONE,
     $                           A( IOFFA+1 ), INT( LDA ),
     $                           A( IOFFA ), INT( LDA ),
     $                           CONE, A( ICURR ), 1 )
