      SUBROUTINE PCPOTRF_I8( UPLO, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
*     INTEGER*8 native version of PCPOTRF.
*     All public integer arguments (N, IA, JA, DESCA) are INTEGER*8;
*     INFO stays INTEGER.
*     Uses I8 calls for PCHERK and PCTRSM; PCPOTF2 stays legacy
...
*  =======
*
*  PCPOTRF_I8 computes the Cholesky factorization of an N-by-N complex
...
*     .. External Subroutines ..
     ...
     $                   PB_TOPGET, PB_TOPSET, PCHK1MAT_I8, PCPOTF2,
     $                   PCHERK_I8, PCTRSM_I8, PXERBLA
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCPOTRF_I8', -INFO )
         RETURN
      END IF
*
...
*
     ...
     $   RETURN
*
*     Range check: PCPOTF2 requires narrowing to INTEGER.
*     N, IA, JA must fit in INTEGER for the POTF2 calls.
*
      IF( N.GT.INTMAX .OR. IA.GT.INTMAX .OR. JA.GT.INTMAX ) THEN
         INFO = -1
         CALL PXERBLA( ICTXT, 'PCPOTRF_I8', 1 )
         RETURN
      END IF
*
*     Narrow descriptor for PCPOTF2 calls (legacy INTEGER interface)
*
...
*
         CALL PCPOTF2( UPLO, JB, A, INT( IA ), INT( JA ), DESCA4,
     $                 INFO )
         IF( INFO.NE.0 )
     $      GO TO 30
*
...
*           Perform unblocked Cholesky factorization on JB block
*
            CALL PCPOTF2( UPLO, JB, A, INT( I ), INT( J ), DESCA4,
     $                    INFO )
            IF( INFO.NE.0 ) THEN
               INFO = INFO + INT( J - JA )
               GO TO 30
...
*              Update the trailing matrix, A = A - U'*U
*
               CALL PCHERK_I8( UPLO, 'Conjugate transpose', N-J-JB+JA,
...
*
            CALL PCTRSM_I8( 'Right', UPLO, 'Conjugate transpose',
     $                      'Non-Unit', N-JB, INT( JB, 8 ), CONE, A,
     $                      IA, JA, DESCA, A, IA+JB, JA, DESCA )
*
*           Update the trailing matrix, A = A - L*L'
*
            CALL PCHERK_I8( UPLO, 'No Transpose', N-JB, INT( JB, 8 ),
...
*              Update the trailing matrix, A = A - L*L'
*
               CALL PCHERK_I8( UPLO, 'No Transpose', N-J-JB+JA,
