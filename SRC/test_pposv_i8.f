      PROGRAM TEST_PPOSV_I8
      IMPLICIT NONE
*
*  Test PxPOSV_I8 — compare against legacy PxPOSV.
*  Verifies POTRF_I8 + POTRS_I8 produce bit-identical solutions.
*
*  Usage:  mpirun -np 4 ./xposv_i8
*
...
*
      IF( IAM .EQ. 0 )
     $   WRITE(*,'(A)') 'test_pposv_i8: 2x2 grid'
*
*     === DOUBLE PRECISION ===
*
      IF( IAM .EQ. 0 ) WRITE(*,'(A)') '--- PDPOSV_I8 (D) ---'
      CALL RUN_DPOSV( 30_8, 4_8, 5_8, ICTXT, NPROW, NPCOL,
     $                MYROW, MYCOL, IAM, 'D:N=30,NB=4 ',
     $                NTEST, NFAIL )
*
*     === REAL ===
*
      IF( IAM .EQ. 0 ) WRITE(*,'(A)') '--- PSPOSV_I8 (S) ---'
      CALL RUN_SPOSV( 30_8, 4_8, 5_8, ICTXT, NPROW, NPCOL,
     $                MYROW, MYCOL, IAM, 'S:N=30,NB=4 ',
     $                NTEST, NFAIL )
*
*     === COMPLEX*16 ===
*
      IF( IAM .EQ. 0 ) WRITE(*,'(A)') '--- PCPOSV_I8 (C) ---'
      CALL RUN_CPOSV( 30_8, 4_8, 5_8, ICTXT, NPROW, NPCOL,
     $                MYROW, MYCOL, IAM, 'C:N=30,NB=4 ',
     $                NTEST, NFAIL )
*
      IF( IAM .EQ. 0 ) WRITE(*,'(A)') '--- PZPOSV_I8 (Z) ---'
      CALL RUN_ZPOSV( 30_8, 4_8, 5_8, ICTXT, NPROW, NPCOL,
...
*
      IF( IAM .EQ. 0 ) THEN
         WRITE(*,'(A)') '======================================'
         WRITE(*,'(A)') 'PxPOSV_I8 Test Summary'
         WRITE(*,'(A)') '======================================'
...
*
*     ================================================================
*     RUN_DPOSV — compare PDPOSV_I8 vs legacy PDPOSV
*     ================================================================
*
      SUBROUTINE RUN_DPOSV( N8, NB8, NRHS8, ICTXT, NPROW, NPCOL,
...
*
*     SPD matrix: A(i,j) = N+1-|i-j|, diag boosted
*
      DO J8 = 1, LCA
...
*
      INFO = 0
      CALL PDPOSV_I8( 'L', N8, NRHS8, A, 1_8, 1_8, DESCA8,
...
*
      INFO = 0
      CALL PDPOSV( 'L', N4, NRHS4, ACOPY, 1, 1, DESCA4,
...
*
*     ================================================================
*     RUN_ZPOSV — compare PZPOSV_I8 vs legacy PZPOSV
*     ================================================================
*
      SUBROUTINE RUN_ZPOSV( N8, NB8, NRHS8, ICTXT, NPROW, NPCOL,
     $                       MYROW, MYCOL, IAM, LABEL, NTEST, NFAIL )
      IMPLICIT NONE
      INCLUDE 'SL_i8_params.inc'
*
...
*
      ...
      ALLOCATE( BREF( MAX( LLDB*LCB, 1_8 ) ) )
*
*     HPD matrix: A(i,j) = (N+1-|i-j|) + i*(i-j), diag real boosted
*
      DO J8 = 1, LCA
         GJ = INDXL2G_I8( J8, NB8, MYCOL, 0, NPCOL )
         DO I8 = 1, LRA
            GI = INDXL2G_I8( I8, NB8, MYROW, 0, NPROW )
            IF( GI .EQ. GJ ) THEN
               A( (J8-1)*LLDA + I8 ) = DCMPLX( 2.0D0*DBLE(N8), 0.0D0 )
            ELSE
...
*
      INFO = 0
      CALL PZPOSV_I8( 'L', N8, NRHS8, A, 1_8, 1_8, DESCA8,
     $                B, 1_8, 1_8, DESCB8, INFO )
*
      NTEST = NTEST + 1
      IF( INFO .NE. 0 ) THEN
...
*
      INFO = 0
      CALL PZPOSV( 'L', N4, NRHS4, ACOPY, 1, 1, DESCA4,
     $             BREF, 1, 1, DESCB4, INFO )
*
*     === Compare solution B ===
*
...
*
*     ================================================================
*     RUN_SPOSV — compare PSPOSV_I8 vs legacy PSPOSV
*     ================================================================
*
      SUBROUTINE RUN_SPOSV( N8, NB8, NRHS8, ICTXT, NPROW, NPCOL,
     $                       MYROW, MYCOL, IAM, LABEL, NTEST, NFAIL )
      IMPLICIT NONE
      INCLUDE 'SL_i8_params.inc'
*
...
*
      ...
      INTEGER            NUMROC
      EXTERNAL           NUMROC
      EXTERNAL           DESCINIT_I8, DESCINIT, PSPOSV_I8, PSPOSV
...
*
      INFO = 0
      CALL PSPOSV_I8( 'L', N8, NRHS8, A, 1_8, 1_8, DESCA8,
     $                B, 1_8, 1_8, DESCB8, INFO )
      NTEST = NTEST + 1
      IF( INFO .NE. 0 ) THEN
         IF( IAM .EQ. 0 )
...
*
      INFO = 0
      CALL PSPOSV( 'L', N4, NRHS4, ACOPY, 1, 1, DESCA4,
     $             BREF, 1, 1, DESCB4, INFO )
*
      ERRS = 0
      NTEST = NTEST + 1
...
*
*     ================================================================
*     RUN_CPOSV — compare PCPOSV_I8 vs legacy PCPOSV
*     ================================================================
*
      SUBROUTINE RUN_CPOSV( N8, NB8, NRHS8, ICTXT, NPROW, NPCOL,
     $                       MYROW, MYCOL, IAM, LABEL, NTEST, NFAIL )
      IMPLICIT NONE
      INCLUDE 'SL_i8_params.inc'
*
...
*
      ...
      INTEGER            NUMROC
      EXTERNAL           NUMROC
      EXTERNAL           DESCINIT_I8, DESCINIT, PCPOSV_I8, PCPOSV
...
*     HPD matrix
      DO J8 = 1, LCA
         ...
         DO I8 = 1, LRA
            GI = INDXL2G_I8( I8, NB8, MYROW, 0, NPROW )
            IF( GI .EQ. GJ ) THEN
               A( (J8-1)*LLDA + I8 ) = CMPLX( 2.0*REAL(N8), 0.0 )
            ELSE
...
*
      INFO = 0
      CALL PCPOSV_I8( 'L', N8, NRHS8, A, 1_8, 1_8, DESCA8,
     $                B, 1_8, 1_8, DESCB8, INFO )
      NTEST = NTEST + 1
      IF( INFO .NE. 0 ) THEN
         IF( IAM .EQ. 0 )
...
*
      INFO = 0
      CALL PCPOSV( 'L', N4, NRHS4, ACOPY, 1, 1, DESCA4,
     $             BREF, 1, 1, DESCB4, INFO )
*
      ERRS = 0
      NTEST = NTEST + 1
