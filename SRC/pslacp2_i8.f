      SUBROUTINE PSLACP2_I8( UPLO, M, N, A, IA, JA, DESCA,
     $                       B, IB, JB, DESCB )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
*     INTEGER*8 version of PSLACP2.
*
*  PSLACP2_I8 copies all or part of a distributed matrix A to another
...
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDINFO, SLAMOV_I8, INFOG2L_I8
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      INTEGER*8          NUMROC_I8
...
*
   10          CONTINUE
               IF( ( N-ITOP ).GT.0 ) THEN
                  CALL SLAMOV_I8( UPLO, IINXTA-IIBEGA+1, N-ITOP,
...
*
   20          CONTINUE
               IF( JJAA.LE.( JJA+N-1 ) ) THEN
                  HEIGHT = IBASE - ITOP
                  CALL SLAMOV_I8( 'All', MPAA, ITOP-JJAA+JJA,
     $                 A( IIAA+(JJAA-1)*LDA ), LDA,
     $                 B( IIBB+(JJBB-1)*LDB ), LDB )
                  CALL SLAMOV_I8( UPLO, MPAA, HEIGHT,
...
*
            ELSE
*
               CALL SLAMOV_I8( 'All', MP, N, A( IIA+(JJA-1)*LDA ),
...
*
   30          CONTINUE
               IF( ( M-ILEFT ).GT.0 ) THEN
                  CALL SLAMOV_I8( UPLO, M-ILEFT, JJNXTA-JJBEGA+1,
...
*
   40          CONTINUE
               IF( IIAA.LE.( IIA+M-1 ) ) THEN
                  WIDE = IRIGHT - ILEFT
                  CALL SLAMOV_I8( 'All', ILEFT-IIAA+IIA, NQAA,
     $                 A( IIAA+(JJAA-1)*LDA ), LDA,
     $                 B( IIBB+(JJBB-1)*LDB ), LDB )
                  CALL SLAMOV_I8( UPLO, WIDE, NQAA,
...
*
            ELSE
*
               CALL SLAMOV_I8( 'All', M, NQ, A( IIA+(JJA-1)*LDA ),
     $                         LDA, B( IIB+(JJB-1)*LDB ), LDB )
*
            END IF
*
