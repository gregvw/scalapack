...
void pzdotu_( n, dotu, X, ix, jx, desc_X, incx, Y, iy, jy, desc_Y,
              incy )
/*
*  .. Scalar Arguments ..
*/
...
*  =======
*
*  PZDOTU forms the dot product of two distributed vectors,
*
*     dotu := sub( X )**T * sub( Y )
*
...
*          N >= 0.
*
*  DOTU    (local output) pointer to COMPLEX*16
   ...
   if( *n == 1 )
   {
         ...
         if( myrow == ixrow )
         {
            ...
            else
            {
               zgebr2d_( &ictxt, C2F_CHAR( ROW ), C2F_CHAR( rbtop ),
                         &ione, &ione, dotu, &ione, &myrow, &ixcol );
            }
         }
      }
         ...
         if( mycol == ixcol )
         {
            ...
            else
            {
               zgebr2d_( &ictxt, C2F_CHAR( COLUMN ), C2F_CHAR( cbtop ),
                         &ione, &ione, dotu, &ione, &ixrow, &mycol );
            }
         }
      }
      ...
      if( ( *incy == desc_Y[M_] ) && ( desc_Y[M_] != 1 ) )
      {
         if( myrow == iyrow )
         {
            ...
            {
               zgebr2d_( &ictxt, C2F_CHAR( ROW ), C2F_CHAR( rbtop ),
                         &ione, &ione, dotu, &ione, &myrow, &iycol );
            }
         }
      }
      else if( ( *incy == 1 ) && ( desc_Y[M_] != 1 ) )
      {
         if( mycol == iycol )
         {
            ...
            {
               zgebr2d_( &ictxt, C2F_CHAR( COLUMN ), C2F_CHAR( cbtop ),
                         &ione, &ione, dotu, &ione, &iyrow, &mycol );
            }
         }
      }
      return;
