      SUBROUTINE PZPOTF2_I8( UPLO, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
*     INTEGER*8 native version of PZPOTF2.
*     All public integer arguments are INTEGER*8; INFO stays INTEGER.
*     Local BLAS calls (ZGEMV, ZDSCAL, ZLACGV, ZZDOTC) use INT()
...
*  =======
*
*  PZPOTF2_I8 computes the Cholesky factorization of a complex
...
*     .. External Subroutines ..
     ...
     $                   PXERBLA, ZGEMV, ZLACGV, ZDSCAL, ZZDOTC
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          DBLE, INT, MOD, SQRT
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZPOTF2_I8', -INFO )
...
*
                  CALL ZZDOTC( INT( J-JA ), DOT, A( IOFFA ), 1,
     $                         A( IOFFA ), 1 )
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + LDA
                     CALL ZLACGV( INT( J-JA ), A( IOFFA ), 1 )
                     CALL ZGEMV( 'Transpose', INT( J-JA ),
     $                           INT( JA+N-J-1 ), -CONE,
     $                           A( IOFFA+LDA ), INT( LDA ),
     $                           A( IOFFA ), 1,
     $                           CONE, A( ICURR ), INT( LDA ) )
                     CALL ZLACGV( INT( J-JA ), A( IOFFA ), 1 )
                     CALL ZDSCAL( INT( JA+N-J-1 ), ONE / AJJ,
     $                            A( ICURR ), INT( LDA ) )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + LDA
...
*
                  CALL ZZDOTC( INT( J-JA ), DOT, A( IOFFA ),
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + 1
                     CALL ZLACGV( INT( J-JA ), A( IOFFA ),
     $                            INT( LDA ) )
                     CALL ZGEMV( 'No transpose', INT( JA+N-J-1 ),
     $                           INT( J-JA ), -CONE,
     $                           A( IOFFA+1 ), INT( LDA ),
     $                           A( IOFFA ), INT( LDA ),
     $                           CONE, A( ICURR ), 1 )
