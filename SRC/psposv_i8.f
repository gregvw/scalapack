      SUBROUTINE PSPOSV_I8( UPLO, N, NRHS, A, IA, JA, DESCA, B, IB,
     $                     JB, DESCB, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
*     INTEGER*8 native version of PSPOSV.
*     All public integer arguments are INTEGER*8 except INFO (INTEGER).
*     Calls PSPOTRF_I8 and PSPOTRS_I8 internally.
*
*     .. Scalar Arguments ..
      CHARACTER          UPLO
...
*  =======
*
*  PSPOSV_I8 computes the solution to a real system of linear equations
*
*                       sub( A ) * X = sub( B ),
*
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPOSV_I8', -INFO )
         RETURN
      END IF
*
*     Compute the Cholesky factorization sub( A ) = U'*U or L*L'.
*
      CALL PSPOTRF_I8( UPLO, N, A, IA, JA, DESCA, INFO )
*
      IF( INFO.EQ.0 ) THEN
*
...
*        with X.
*
         CALL PSPOTRS_I8( UPLO, N, NRHS, A, IA, JA, DESCA, B, IB, JB,
     $                    DESCB, INFO )
*
      END IF
*
