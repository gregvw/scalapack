      SUBROUTINE PSPOTF2_I8( UPLO, N, A, IA, JA, DESCA, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
*     INTEGER*8 native version of PSPOTF2.
*     All public integer arguments are INTEGER*8; INFO stays INTEGER.
*     Local BLAS calls (SGEMV, SSCAL, SDOT) use INT() narrowing
...
*  =======
*
*  PSPOTF2_I8 computes the Cholesky factorization of a real symmetric
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSPOTF2_I8', -INFO )
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + LDA
                     CALL SGEMV( 'Transpose', INT( J-JA ),
     $                           INT( JA+N-J-1 ), -ONE,
     $                           A( IOFFA+LDA ), INT( LDA ),
     $                           A( IOFFA ), 1,
     $                           ONE, A( ICURR ), INT( LDA ) )
                     CALL SSCAL( INT( N-J+JA-1 ), ONE / AJJ,
     $                           A( ICURR ), INT( LDA ) )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + LDA
...
*
                  IF( J.LT.JA+N-1 ) THEN
                     ICURR = IDIAG + 1
                     CALL SGEMV( 'No transpose', INT( JA+N-J-1 ),
     $                           INT( J-JA ), -ONE,
     $                           A( IOFFA+1 ), INT( LDA ),
     $                           A( IOFFA ), INT( LDA ),
     $                           ONE, A( ICURR ), 1 )
                     CALL SSCAL( INT( JA+N-J-1 ), ONE / AJJ,
     $                           A( ICURR ), 1 )
                  END IF
                  IDIAG = IDIAG + LDA + 1
                  IOFFA = IOFFA + 1
