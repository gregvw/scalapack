      SUBROUTINE PZGETRF_I8( M, N, A, IA, JA, DESCA, IPIV, INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PZGETRF_I8', -INFO )
         RETURN
      END IF
*
...
*
      IF( JB+1.LE.N ) THEN
*
         CALL PZLASWP_I8( 'Forward', 'Rows', N-INT( JB, 8 ),
     $                    A, IA, JN+1, DESCA, IA, IN, IPIV )
*
         CALL PZTRSM_I8( 'Left', 'Lower', 'No transpose', 'Unit',
     $                   INT( JB, 8 ), N-INT( JB, 8 ), ONE,
     $                   A, IA, JA, DESCA, A, IA, JN+1, DESCA )
*
         IF( JB+1.LE.M ) THEN
*
            CALL PZGEMM_I8( 'No transpose', 'No transpose',
...
*
     ...
     $      INFO = IINFO + INT( J - JA )
*
         CALL PZLASWP_I8( 'Forward', 'Rowwise', J-JA,
     $                    A, IA, JA, DESCA, I, I+INT( JB, 8 )-1,
     $                    IPIV )
*
         IF( J-JA+JB+1.LE.N ) THEN
*
            CALL PZLASWP_I8( 'Forward', 'Rowwise',
     $                       N-J-INT( JB, 8 )+JA,
