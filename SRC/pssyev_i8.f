...
*
*     INTEGER*8 native version of PSSYEV.
*     All public integer arguments are INTEGER*8; INFO stays INTEGER.
*     Uses I8 calls internally; PSLANSY is the one remaining narrowing
...
*  =======
*
*  PSSYEV_I8 computes all eigenvalues and, optionally, eigenvectors
*  of a real symmetric matrix A by calling the recommended sequence
*  of ScaLAPACK routines.  This is the INTEGER*8 version of PSSYEV.
*
*  =====================================================================
*
*     .. Parameters ..
...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME, NUMROC_I8, PSLAMCH, PSLANSY,
     $                   SL_GRIDRESHAPE, INDXG2P_I8
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_GRIDEXIT, BLACS_GRIDINFO,
     $                   CHK1MAT_I8, SCOPY,
     $                   DESCINIT_I8, SSCAL, SSTEQR2,
     $                   NARROW_DESC8,
     $                   PCHK1MAT_I8, PCHK2MAT_I8,
     $                   PSELGET_I8, PSGEMR2D_I8,
     $                   PSLASCL_I8, PSLASET_I8, PSORMTR_I8,
     $                   PSSYNTRD_I8, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, ICHAR, INT, MAX, MIN, MOD, REAL, SQRT
*     ..
...
*           matrix required by SSTEQR2. This specific distribution
*           allows each process to do 1/pth of the work updating matrix
*           Q during SSTEQR2 and achieve some parallelization to an
...
*
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PSSYEV_I8', -INFO )
...
*     PSLANSY has no _I8 version -- narrow DESCA for this call only.
*
      ISCALE = 0
*
      CALL NARROW_DESC8( DESCA, DESCA4 )
      ANRM = PSLANSY( 'M', UPLO, INT( N ), A, INT( IA ), INT( JA ),
     $                 DESCA4, WORK( INDWORK ) )
*
...
*
      IF( ISCALE.EQ.1 ) THEN
         CALL PSLASCL_I8( UPLO, ONE, SIGMA, N, N, A, IA, JA,
     $                     DESCA, IINFO )
      END IF
*
*     Reduce symmetric matrix to tridiagonal form.
*
      CALL PSSYNTRD_I8( UPLO, N, A, IA, JA, DESCA, WORK( INDD ),
...
*
      DO 10 I8 = 1, N
         CALL PSELGET_I8( 'A', ' ', WORK( INDD2+I8-1 ), A,
     $                    I8+IA-1, I8+JA-1, DESCA )
 10   CONTINUE
      IF( LSAME( UPLO, 'U') ) THEN
          DO 20 I8 = 1, N-1
             CALL PSELGET_I8( 'A', ' ', WORK( INDE2+I8-1 ), A,
     $                        I8+IA-1, I8+JA, DESCA )
 20       CONTINUE
      ELSE
          DO 30 I8 = 1, N-1
...
*
      IF( WANTZ ) THEN
*
         CALL PSLASET_I8( 'Full', N, N, ZERO, ONE, WORK( INDWORK ),
     $                    1_8, 1_8, DESCQR )
*
*        SSTEQR2 is a modified version of LAPACK's SSTEQR.  The
*        modifications allow each process to perform partial updates
*        to matrix Q.
*
         CALL SSTEQR2( 'I', INT( N ), WORK( INDD2 ), WORK( INDE2 ),
     $                 WORK( INDWORK ), INT( LDC ), INT( NRC ),
     $                 WORK( INDWORK2 ), INFO )
*
         CALL PSGEMR2D_I8( N, N, WORK( INDWORK ), 1_8, 1_8, DESCQR,
     $                     Z, IA, JA, DESCZ, INT( CONTEXTC, 8 ) )
*
         CALL PSORMTR_I8( 'L', UPLO, 'N', N, N, A, IA, JA, DESCA,
     $                    WORK( INDTAU ), Z, IZ, JZ, DESCZ,
     $                    WORK( INDWORK ), LLWORK, IINFO )
*
      ELSE
*
         CALL SSTEQR2( 'N', INT( N ), WORK( INDD2 ), WORK( INDE2 ),
...
*
      CALL SCOPY( INT( N ), WORK( INDD2 ), 1, W, 1 )
*
...
*
 ...
 40   CONTINUE
*
      CALL SGAMN2D( ICTXT, 'a', ' ', INT( J8 ), 1,
     $              WORK( 1+INDTAU ), INT( J8 ), 1, 1, -1, -1, 0 )
      CALL SGAMX2D( ICTXT, 'a', ' ', INT( J8 ), 1,
