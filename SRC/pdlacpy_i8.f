      SUBROUTINE PDLACPY_I8( UPLO, M, N, A, IA, JA, DESCA,
     $                       B, IB, JB, DESCB )
      IMPLICIT NONE
*
*  -- ScaLAPACK auxiliary routine --
*     INTEGER*8 version of PDLACPY.
*
*  PDLACPY_I8 copies all or part of a distributed matrix A to another
*  distributed matrix B.  No communication is performed; this is a
*  local copy that decomposes the operation into block-aligned
*  sub-copies handled by PDLACP2_I8.
*
*     .. Scalar Arguments ..
      CHARACTER          UPLO
...
*
     ...
     $    N.LE.( NBA - MOD( JA-1, NBA ) ) ) THEN
         CALL PDLACP2_I8( UPLO, M, N, A, IA, JA, DESCA,
     $                    B, IB, JB, DESCB )
      ELSE
*
         IF( LSAME( UPLO, 'U' ) ) THEN
            CALL PDLACP2_I8( UPLO, IN-IA+1, N, A, IA, JA, DESCA,
     $                       B, IB, JB, DESCB )
            DO 10 ITMP = IN+1-IA, M-1, MBA
               ...
               JBB = JB + ITMP
               JAA = JA + ITMP
               CALL PDLACP2_I8( UPLO, IBLK, N-ITMP,
     $                          A, IA+ITMP, JAA, DESCA,
     $                          B, IBB, JBB, DESCB )
   10       CONTINUE
         ELSE IF( LSAME( UPLO, 'L' ) ) THEN
            CALL PDLACP2_I8( UPLO, M, JN-JA+1, A, IA, JA, DESCA,
     $                       B, IB, JB, DESCB )
            DO 20 JTMP = JN+1-JA, N-1, NBA
               ...
               JBB = JB + JTMP
               IAA = IA + JTMP
               CALL PDLACP2_I8( UPLO, M-JTMP, JBLK,
     $                          A, IAA, JA+JTMP, DESCA,
     $                          B, IBB, JBB, DESCB )
   20       CONTINUE
         ELSE
