      SUBROUTINE PSORMLQ_I8( SIDE, TRANS, M, N, K, A, IA, JA, DESCA,
     $                       TAU, C, IC, JC, DESCC, WORK, LWORK,
     $                       INFO )
      IMPLICIT NONE
*
*  -- ScaLAPACK routine --
*     INTEGER*8 thin wrapper around PSORMLQ.
*
*  Overwrites the general real distributed M-by-N matrix sub( C )
*  with Q*sub(C) or Q'*sub(C) or sub(C)*Q or sub(C)*Q'.
*
*     .. Scalar Arguments ..
...
*     .. External Subroutines ..
      EXTERNAL           NARROW_DESC8, PSORMLQ, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          INT
*     ..
...
*
     ...
     $    IC.GT.INTMAX .OR. JC.GT.INTMAX ) THEN
         INFO = -1
         CALL PXERBLA( ICTXT, 'PSORMLQ_I8', -1 )
         RETURN
      END IF
*
      M4  = INT( M )
...
*
      CALL PSORMLQ( SIDE, TRANS, M4, N4, K4, A, IA4, JA4, DESCA4,
