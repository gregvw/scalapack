...
intersect(char *uplo, char *diag,
	  ...
	  Int action,
	  Int *ptrsizebuff, float **pptrbuff, float *ptrblock,
	  Int m, Int n,
 ...
 * also contigous in the real one ! */
{
  ...
  if (!ScaLAPACK_RedistApiAddToApiInt(*ptrsizebuff, intervalsize, ptrsizebuff)) {
    fprintf(stderr, "pstrmr2: interval element count overflow\n");
    exit(1);
  }
  switch (action) {
  case SENDBUFF:	/* fill buff with local elements to be sent */
    ...
    if (!ScaLAPACK_Index64ToSizeT((ScaLAPACK_Index64) intervalsize, &copy_count) ||
        !ScaLAPACK_SizeTMul(copy_count, sizeof(float), &copy_bytes)) {
      fprintf(stderr, "pstrmr2: interval byte count overflow\n");
      exit(1);
    }
...
void
scanD0(char *uplo, char *diag, Int action, float *ptrbuff, Int *ptrsizebuff,
       Int m, Int n,
