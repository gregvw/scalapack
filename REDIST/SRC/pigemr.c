...
#if defined(Add_) || defined(f77IsF2C)
#define fortran_mr2dnew_i8 pigemr2d_i8_
#elif defined(UpCase)
#define fortran_mr2dnew_i8 PIGEMR2D_I8
#else
#define fortran_mr2dnew_i8 pigemr2d_i8
#endif
...
extern void Cpigemr2d_core( ScaLAPACK_Index64 m, ScaLAPACK_Index64 n, Int *ptrmyblock, ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja, MDESC_CORE *ma, Int *ptrmynewblock, ScaLAPACK_Index64 ib, ScaLAPACK_Index64 jb, MDESC_CORE *mb, int globcontext );
...
/* Cpigemr2d: thin wrapper — unpacks MDESC to MDESC_CORE and delegates */
void
	  ...
	  Int globcontext)
{
  ...
  Cpigemr2d_core((ScaLAPACK_Index64)m, (ScaLAPACK_Index64)n,
		 ptrmyblock,
...
block2buff_core(IDESC_CORE *vi, ScaLAPACK_Index64 vinb,
		...
		Int *ptra, const MDESC_CORE *ma, Int *buff)
{
...
buff2block_core(IDESC_CORE *vi, ScaLAPACK_Index64 vinb,
		IDESC_CORE *hi, ScaLAPACK_Index64 hinb,
		Int *buff, Int *ptrb, const MDESC_CORE *mb)
{
  ScaLAPACK_Index64 h, v, sizebuff;
  sizebuff = 0;
  for (h = 0; h < hinb; h++) {
...
/* ------------------------------------------------------------------ */
/* Cpigemr2d_core — full redistribution with Index64 dimensions       */
/* ------------------------------------------------------------------ */
...
Cpigemr2d_core(ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	       ...
	       int globcontext)
{
  Int *ptrsendbuff = NULL, *ptrrecvbuff = NULL;
  Int *recvptr;
		   ...
		   int64_t *gcontext)
{
  ...
  if (!unpack_desc_i8(desc_A, &core_a)) {
    fprintf(stderr, "PIGEMR2D_I8:bad source descriptor (narrowing overflow)\n");
    exit(1);
  }
  if (!unpack_desc_i8(desc_B, &core_b)) {
    fprintf(stderr, "PIGEMR2D_I8:bad destination descriptor (narrowing overflow)\n");
    exit(1);
  }

  if (*gcontext < INT_MIN || *gcontext > INT_MAX) {
    fprintf(stderr, "PIGEMR2D_I8:bad global context (narrowing overflow)\n");
    exit(1);
  }
  gc = (int)*gcontext;

  Cpigemr2d_core(*m, *n, A, *ia, *ja, &core_a,
		 B, *ib, *jb, &core_b, gc);
}
