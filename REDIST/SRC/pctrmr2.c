...
intersect(char *uplo, char *diag,
	  ...
	  Int action,
	  Int *ptrsizebuff, complex **pptrbuff, complex *ptrblock,
	  Int m, Int n,
 ...
 * also contigous in the real one ! */
{
  ...
  if (!ScaLAPACK_RedistApiAddToApiInt(*ptrsizebuff, intervalsize, ptrsizebuff)) {
    fprintf(stderr, "pctrmr2: interval element count overflow\n");
    exit(1);
  }
  switch (action) {
  case SENDBUFF:	/* fill buff with local elements to be sent */
    ...
    if (!ScaLAPACK_Index64ToSizeT((ScaLAPACK_Index64) intervalsize, &copy_count) ||
        !ScaLAPACK_SizeTMul(copy_count, sizeof(complex), &copy_bytes)) {
      fprintf(stderr, "pctrmr2: interval byte count overflow\n");
      exit(1);
    }
...
void
scanD0(char *uplo, char *diag, Int action, complex *ptrbuff, Int *ptrsizebuff,
       Int m, Int n,
