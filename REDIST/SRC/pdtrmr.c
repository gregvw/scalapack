...
#if defined(Add_) || defined(f77IsF2C)
#define fortran_mr2dnew_i8 pdtrmr2d_i8_
#elif defined(UpCase)
#define fortran_mr2dnew_i8 PDTRMR2D_I8
#else
#define fortran_mr2dnew_i8 pdtrmr2d_i8
#endif
...
extern void Cpdtrmr2d_core( char* uplo, char* diag, ScaLAPACK_Index64 m, ScaLAPACK_Index64 n, double *ptrmyblock, ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja, MDESC_CORE *ma, double *ptrmynewblock, ScaLAPACK_Index64 ib, ScaLAPACK_Index64 jb, MDESC_CORE *mb, int globcontext );
...
intersect_core(const char *uplo, const char *diag,
	       ...
	       double **pptrbuff, double *ptrblock,
	       ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	       const MDESC_CORE *ma,
	       ...
	       ScaLAPACK_Index64 templatewidth1)
{
  ...
  case SENDBUFF:
	...
	!ScaLAPACK_SizeTMul(copy_bytes, sizeof(double), &copy_bytes)) {
...
scanD0_core(const char *uplo, const char *diag,
	    int action,
	    double *ptrbuff, ScaLAPACK_Index64 *ptrsizebuff,
	    ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	    const MDESC_CORE *ma,
	    ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja,
	    int p0, int q0,
...
/* ------------------------------------------------------------------ */
/* Cpdtrmr2d_core — full triangular redistribution with Index64 dims  */
/* ------------------------------------------------------------------ */

void
Cpdtrmr2d_core(char *uplo, char *diag,
	       ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	       double *ptrmyblock,
	       ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja,
	       MDESC_CORE *ma,
	       ...
	       int globcontext)
{
  double *ptrsendbuff = NULL, *ptrrecvbuff = NULL, *ptrNULL = 0;
  double *recvptr;
		   ...
		   int64_t *gcontext)
{
  ...
  if (!unpack_desc_i8(desc_A, &core_a)) {
    fprintf(stderr, "PDTRMR2D_I8:bad source descriptor (narrowing overflow)\n");
    exit(1);
  }
  if (!unpack_desc_i8(desc_B, &core_b)) {
    fprintf(stderr, "PDTRMR2D_I8:bad destination descriptor (narrowing overflow)\n");
    exit(1);
  }

  if (*gcontext < INT_MIN || *gcontext > INT_MAX) {
    fprintf(stderr, "PDTRMR2D_I8:bad global context (narrowing overflow)\n");
    exit(1);
  }
  gc = (int)*gcontext;

  Cpdtrmr2d_core(uplo, diag, *m, *n, A, *ia, *ja, &core_a,
		 B, *ib, *jb, &core_b, gc);
}
