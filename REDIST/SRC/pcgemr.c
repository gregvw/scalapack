...
#if defined(Add_) || defined(f77IsF2C)
#define fortran_mr2dnew_i8 pcgemr2d_i8_
#elif defined(UpCase)
#define fortran_mr2dnew_i8 PCGEMR2D_I8
#else
#define fortran_mr2dnew_i8 pcgemr2d_i8
#endif
typedef struct {
...
extern void Cpcgemr2d_core( ScaLAPACK_Index64 m, ScaLAPACK_Index64 n, complex *ptrmyblock, ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja, MDESC_CORE *ma, complex *ptrmynewblock, ScaLAPACK_Index64 ib, ScaLAPACK_Index64 jb, MDESC_CORE *mb, int globcontext );
...
/* Cpcgemr2d: thin wrapper — unpacks MDESC to MDESC_CORE and delegates */
void
Cpcgemr2d(Int m, Int n,
	  complex *ptrmyblock, Int ia, Int ja, MDESC *ma,
	  complex *ptrmynewblock, Int ib, Int jb, MDESC *mb,
	  Int globcontext)
{
  ...
  Cpcgemr2d_core((ScaLAPACK_Index64)m, (ScaLAPACK_Index64)n,
		 ptrmyblock,
...
block2buff_core(IDESC_CORE *vi, ScaLAPACK_Index64 vinb,
		...
		complex *ptra, const MDESC_CORE *ma, complex *buff)
{
...
buff2block_core(IDESC_CORE *vi, ScaLAPACK_Index64 vinb,
		IDESC_CORE *hi, ScaLAPACK_Index64 hinb,
		complex *buff, complex *ptrb, const MDESC_CORE *mb)
{
  ScaLAPACK_Index64 h, v, sizebuff;
  sizebuff = 0;
  for (h = 0; h < hinb; h++) {
...
/* ------------------------------------------------------------------ */
/* Cpcgemr2d_core — full redistribution with Index64 dimensions       */
/* ------------------------------------------------------------------ */
...
Cpcgemr2d_core(ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	       ...
	       int globcontext)
{
  complex *ptrsendbuff = NULL, *ptrrecvbuff = NULL;
  complex *recvptr;
		   ...
		   int64_t *gcontext)
{
  ...
  if (!unpack_desc_i8(desc_A, &core_a)) {
    fprintf(stderr, "PCGEMR2D_I8:bad source descriptor (narrowing overflow)\n");
    exit(1);
  }
  if (!unpack_desc_i8(desc_B, &core_b)) {
    fprintf(stderr, "PCGEMR2D_I8:bad destination descriptor (narrowing overflow)\n");
    exit(1);
  }

  if (*gcontext < INT_MIN || *gcontext > INT_MAX) {
    fprintf(stderr, "PCGEMR2D_I8:bad global context (narrowing overflow)\n");
    exit(1);
  }
  gc = (int)*gcontext;

  Cpcgemr2d_core(*m, *n, A, *ia, *ja, &core_a,
		 B, *ib, *jb, &core_b, gc);
}
