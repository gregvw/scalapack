...
#if defined(Add_) || defined(f77IsF2C)
#define fortran_mr2dnew_i8 pitrmr2d_i8_
#elif defined(UpCase)
#define fortran_mr2dnew_i8 PITRMR2D_I8
#else
#define fortran_mr2dnew_i8 pitrmr2d_i8
#endif
...
extern void Cpitrmr2d_core( char* uplo, char* diag, ScaLAPACK_Index64 m, ScaLAPACK_Index64 n, Int *ptrmyblock, ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja, MDESC_CORE *ma, Int *ptrmynewblock, ScaLAPACK_Index64 ib, ScaLAPACK_Index64 jb, MDESC_CORE *mb, int globcontext );
...
static void
setmemory_core(Int **adpointer, ScaLAPACK_Index64 blocksize)
{
  size_t alloc_bytes;
  assert(blocksize >= 0);
  if (blocksize == 0) {
  ...
  if (!ScaLAPACK_Index64ToSizeT(blocksize, &alloc_bytes) ||
      !ScaLAPACK_SizeTMul(alloc_bytes, sizeof(Int), &alloc_bytes)) {
...
Clacpy_core(ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	    Int *a, ScaLAPACK_Index64 lda,
	    Int *b, ScaLAPACK_Index64 ldb)
{
	       ...
	       ScaLAPACK_Index64 templatewidth1)
{
  ...
  case SENDBUFF:
	...
	!ScaLAPACK_SizeTMul(copy_bytes, sizeof(Int), &copy_bytes)) {
...
scanD0_core(const char *uplo, const char *diag,
	    int action,
	    Int *ptrbuff, ScaLAPACK_Index64 *ptrsizebuff,
	    ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	    const MDESC_CORE *ma,
	    ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja,
	    int p0, int q0,
...
/* ------------------------------------------------------------------ */
/* Cpitrmr2d_core — full triangular redistribution with Index64 dims  */
/* ------------------------------------------------------------------ */

void
Cpitrmr2d_core(char *uplo, char *diag,
	       ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	       Int *ptrmyblock,
	       ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja,
	       MDESC_CORE *ma,
	       ...
	       int globcontext)
{
  Int *ptrsendbuff = NULL, *ptrrecvbuff = NULL, *ptrNULL = 0;
  Int *recvptr;
  ...
  recvptr = ptrrecvbuff;
  {
	...
	if (mesending && recver[step] >= 0 && (sens == myrang > step)) {
	  ...
	  if (sendsize > 0 && (step != myrang || !merecving)) {
	    ...
	    Cigesd2d(gcontext, ss, (Int)1, ptrsendbuff, ss,
	...
	if (merecving && sender[step] >= 0 && (sens == myrang <= step)) {
	  ...
	  if (recvsize > 0) {
	    if (step == myrang && mesending) {
	      ...
	      Cigerv2d(gcontext, rs, (Int)1, ptrrecvbuff, rs,
...
fortran_mr2dnew_i8(char *uplo, char *diag,
		   int64_t *m, int64_t *n,
		   Int *A, int64_t *ia, int64_t *ja, int64_t desc_A[9],
		   Int *B, int64_t *ib, int64_t *jb, int64_t desc_B[9],
		   int64_t *gcontext)
{
  MDESC_CORE core_a, core_b;
  int gc;
  ...
  if (!unpack_desc_i8(desc_A, &core_a)) {
    fprintf(stderr, "PITRMR2D_I8:bad source descriptor (narrowing overflow)\n");
    exit(1);
  }
  if (!unpack_desc_i8(desc_B, &core_b)) {
    fprintf(stderr, "PITRMR2D_I8:bad destination descriptor (narrowing overflow)\n");
    exit(1);
  }

  if (*gcontext < INT_MIN || *gcontext > INT_MAX) {
    fprintf(stderr, "PITRMR2D_I8:bad global context (narrowing overflow)\n");
    exit(1);
  }
  gc = (int)*gcontext;

  Cpitrmr2d_core(uplo, diag, *m, *n, A, *ia, *ja, &core_a,
		 B, *ib, *jb, &core_b, gc);
}
