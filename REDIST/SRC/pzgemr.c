...
#if defined(Add_) || defined(f77IsF2C)
#define fortran_mr2dnew_i8 pzgemr2d_i8_
#elif defined(UpCase)
#define fortran_mr2dnew_i8 PZGEMR2D_I8
#else
#define fortran_mr2dnew_i8 pzgemr2d_i8
#endif
typedef struct {
...
extern void Cpzgemr2d_core( ScaLAPACK_Index64 m, ScaLAPACK_Index64 n, dcomplex *ptrmyblock, ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja, MDESC_CORE *ma, dcomplex *ptrmynewblock, ScaLAPACK_Index64 ib, ScaLAPACK_Index64 jb, MDESC_CORE *mb, int globcontext );
...
/* Cpzgemr2d: thin wrapper — unpacks MDESC to MDESC_CORE and delegates */
void
Cpzgemr2d(Int m, Int n,
	  dcomplex *ptrmyblock, Int ia, Int ja, MDESC *ma,
	  dcomplex *ptrmynewblock, Int ib, Int jb, MDESC *mb,
	  Int globcontext)
{
  ...
  Cpzgemr2d_core((ScaLAPACK_Index64)m, (ScaLAPACK_Index64)n,
		 ptrmyblock,
...
static void
setmemory_core(dcomplex **adpointer, ScaLAPACK_Index64 blocksize)
{
  size_t alloc_bytes;
  assert(blocksize >= 0);
  if (blocksize == 0) {
  ...
  if (!ScaLAPACK_Index64ToSizeT(blocksize, &alloc_bytes) ||
      !ScaLAPACK_SizeTMul(alloc_bytes, sizeof(dcomplex), &alloc_bytes)) {
...
block2buff_core(IDESC_CORE *vi, ScaLAPACK_Index64 vinb,
		...
		dcomplex *ptra, const MDESC_CORE *ma, dcomplex *buff)
{
...
buff2block_core(IDESC_CORE *vi, ScaLAPACK_Index64 vinb,
		IDESC_CORE *hi, ScaLAPACK_Index64 hinb,
		dcomplex *buff, dcomplex *ptrb, const MDESC_CORE *mb)
{
  ScaLAPACK_Index64 h, v, sizebuff;
  sizebuff = 0;
  for (h = 0; h < hinb; h++) {
...
Clacpy_core(ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	    dcomplex *a, ScaLAPACK_Index64 lda,
	    dcomplex *b, ScaLAPACK_Index64 ldb)
{
...
/* ------------------------------------------------------------------ */
/* Cpzgemr2d_core — full redistribution with Index64 dimensions       */
/* ------------------------------------------------------------------ */
...
Cpzgemr2d_core(ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	       ...
	       int globcontext)
{
  dcomplex *ptrsendbuff = NULL, *ptrrecvbuff = NULL;
  dcomplex *recvptr;
...

  {
    ...
    ptrmyblock = (dcomplex *) ((char *) ptrmyblock + shift_bytes);
    ...
    ptrmynewblock = (dcomplex *) ((char *) ptrmynewblock + shift_bytes);
  ...
  recvptr = ptrrecvbuff;
  {
	...
	if (mesending && recver[step] >= 0 && (sens == myrang > step)) {
	  ...
	  if (sendsize > 0 && (step != myrang || !merecving)) {
	    ...
	    Czgesd2d(gcontext, ss, (Int)1, ptrsendbuff, ss,
	...
	if (merecving && sender[step] >= 0 && (sens == myrang <= step)) {
	  ...
	  if (recvsize > 0) {
	    if (step == myrang && mesending) {
	      ...
	      Czgerv2d(gcontext, rs, (Int)1, ptrrecvbuff, rs,
...
fortran_mr2dnew_i8(int64_t *m, int64_t *n,
		   dcomplex *A, int64_t *ia, int64_t *ja, int64_t desc_A[9],
		   dcomplex *B, int64_t *ib, int64_t *jb, int64_t desc_B[9],
		   int64_t *gcontext)
{
  MDESC_CORE core_a, core_b;
  int gc;
  ...
  if (!unpack_desc_i8(desc_A, &core_a)) {
    fprintf(stderr, "PZGEMR2D_I8:bad source descriptor (narrowing overflow)\n");
    exit(1);
  }
  if (!unpack_desc_i8(desc_B, &core_b)) {
    fprintf(stderr, "PZGEMR2D_I8:bad destination descriptor (narrowing overflow)\n");
    exit(1);
  }

  if (*gcontext < INT_MIN || *gcontext > INT_MAX) {
    fprintf(stderr, "PZGEMR2D_I8:bad global context (narrowing overflow)\n");
    exit(1);
  }
  gc = (int)*gcontext;

  Cpzgemr2d_core(*m, *n, A, *ia, *ja, &core_a,
		 B, *ib, *jb, &core_b, gc);
}
