...
#if defined(Add_) || defined(f77IsF2C)
#define fortran_mr2dnew_i8 pstrmr2d_i8_
#elif defined(UpCase)
#define fortran_mr2dnew_i8 PSTRMR2D_I8
#else
#define fortran_mr2dnew_i8 pstrmr2d_i8
#endif
...
extern void Cpstrmr2d_core( char* uplo, char* diag, ScaLAPACK_Index64 m, ScaLAPACK_Index64 n, float *ptrmyblock, ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja, MDESC_CORE *ma, float *ptrmynewblock, ScaLAPACK_Index64 ib, ScaLAPACK_Index64 jb, MDESC_CORE *mb, int globcontext );
	       ...
	       ScaLAPACK_Index64 templatewidth1)
{
  ...
  case SENDBUFF:
	...
	!ScaLAPACK_SizeTMul(copy_bytes, sizeof(float), &copy_bytes)) {
...
scanD0_core(const char *uplo, const char *diag,
	    int action,
	    float *ptrbuff, ScaLAPACK_Index64 *ptrsizebuff,
	    ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	    const MDESC_CORE *ma,
	    ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja,
	    int p0, int q0,
...
/* ------------------------------------------------------------------ */
/* Cpstrmr2d_core — full triangular redistribution with Index64 dims  */
/* ------------------------------------------------------------------ */

void
Cpstrmr2d_core(char *uplo, char *diag,
	       ScaLAPACK_Index64 m, ScaLAPACK_Index64 n,
	       float *ptrmyblock,
	       ScaLAPACK_Index64 ia, ScaLAPACK_Index64 ja,
	       MDESC_CORE *ma,
	       ...
	       int globcontext)
{
  float *ptrsendbuff = NULL, *ptrrecvbuff = NULL, *ptrNULL = 0;
  float *recvptr;
		   ...
		   int64_t *gcontext)
{
  ...
  if (!unpack_desc_i8(desc_A, &core_a)) {
    fprintf(stderr, "PSTRMR2D_I8:bad source descriptor (narrowing overflow)\n");
    exit(1);
  }
  if (!unpack_desc_i8(desc_B, &core_b)) {
    fprintf(stderr, "PSTRMR2D_I8:bad destination descriptor (narrowing overflow)\n");
    exit(1);
  }

  if (*gcontext < INT_MIN || *gcontext > INT_MAX) {
    fprintf(stderr, "PSTRMR2D_I8:bad global context (narrowing overflow)\n");
    exit(1);
  }
  gc = (int)*gcontext;

  Cpstrmr2d_core(uplo, diag, *m, *n, A, *ia, *ja, &core_a,
		 B, *ib, *jb, &core_b, gc);
}
