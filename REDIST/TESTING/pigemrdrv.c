#include "redist.h"
/* $Id: pigemrdrv.c,v 1.1.1.1 2000/02/15 18:04:10 susan Exp $
 * 
 * pigemrdrv.c :
 * 
 * 
 * PURPOSE:
 * 
 * this driver is testing the PIGEMR2D routine. It calls it to obtain a new
 * scattered block data decomposition of a distributed INTEGER (block
 * scattered) matrix. Then it calls PIGEMR2D for the inverse redistribution
 ...
 * 
 * See pigemr.c file for detailed info on the PIGEMR2D function.
 * 
 * 
 ...
 * 
 * pigemr.o pigemr2.o pigemraux.o
 * 
 * 
 * ======================================
 * 
...
initforpvm(Int argc, char *argv[])
{
  ...
  if (nproc < 1) {	/* we are with PVM */
    if (pnum == 0) {
      if (argc < 2) {
	fprintf(stderr, "usage with PVM:xigemr nbproc\n\
...
%3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT " %3" SLINT_FMT "",
	...
	if (ptrmyblockvide[i] != ptrmyblockcopy[i]) {
	  nberrors++;
	  printf("Proc " SLINT_FMT " : Error element number " SLINT_FMT ", value = " SLINT_FMT " , initvalue =" SLINT_FMT " \n"
		 ,mypnum, i, ptrmyblockvide[i], ptrmyblockcopy[i]);
	};
      };
      if (nberrors > 0) {
	printf("Processor " SLINT_FMT ", has tested  " SLINT_FMT " INTEGER elements,\
