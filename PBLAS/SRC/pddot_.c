...
*  =======
*
*  PDDOT  forms the dot product of two subvectors,
*
*     DOT := sub( X )**T * sub( Y ),
*
*  where
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PDDOT", "X", *N, 1, Xi, Xj, Xd, *INCX,  6, &info );
      PB_Cchkvec( ctxt, "PDDOT", "Y", *N, 1, Yi, Yj, Yd, *INCY, 11, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PDDOT", info ); return; }
#endif
   DOT[REAL_PART] = ZERO;
/*
...
*/
      type = PB_Cdtypeset(); size = type->size; dot = type->Fvvdotu;
/*
