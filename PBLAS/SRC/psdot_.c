...
*  =======
*
*  PSDOT  forms the dot product of two subvectors,
*
*     DOT := sub( X )**T * sub( Y ),
*
*  where
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PSDOT", "X", *N, 1, Xi, Xj, Xd, *INCX,  6, &info );
      PB_Cchkvec( ctxt, "PSDOT", "Y", *N, 1, Yi, Yj, Yd, *INCY, 11, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PSDOT", info ); return; }
#endif
   DOT[REAL_PART] = ZERO;
/*
...
*/
      type = PB_Cstypeset(); size = type->size; dot = type->Fvvdotu;
/*
...
*/
         if( ( XisR && YisR ) || ( XmyprocR == XprocR ) )
         {
            ...
            Csgsum2d( ctxt, &scope, top, 1, 1, ((char *) DOT), 1, -1, 0 );
         }
...
*/
            ...
            if( XnpD > 0 )
            {
               ...
               if( ( myrow != rsrc ) || ( mycol != csrc ) )
               {
                  if( !PB_CSizeMul2( XnpD, size, &alloc_bytes ) )
                     PB_Cabort( ctxt, "PSDOT", -1 );
                  buf = PB_Cmalloc64( alloc_bytes );
                  if( YisRow )
                     Csgerv2d( ctxt, 1, XnpD, buf,    1, rsrc, csrc );
                  else
                     Csgerv2d( ctxt, XnpD, 1, buf, XnpD, rsrc, csrc );
                  dot( &XnpD, ((char *) DOT), Mptr( ((char *) X), Xii, Xjj, Xld,
                       size ), &Xlinc, buf, &ione );
                  if( buf ) free( buf );
               }
            ...
            if( XisRow )
            {
               ...
               Csgsum2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1, -1, 0 );
            }
            else
            {
               top = PB_Ctop( &ctxt, COMBINE, COLUMN, TOP_GET );
               Csgsum2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1, -1, 0 );
            }
         }
      }
      else
...
*/
         if( XisRow )
         {
           ...
           if( XmyprocR == XprocR )
              Csgebs2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1 );
           else
              Csgebr2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1, XprocR,
                        XmyprocD );
         }
         else
         {
           ...
           if( XmyprocR == XprocR )
              Csgebs2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1 );
           else
              Csgebr2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1, XmyprocD,
                        XprocR );
         }
      }
      else
...
*/
            if( XprocR != YprocR )
            {
               ...
               {
                  if( XisRow )
                     Csgesd2d( ctxt, 1, 1, ((char *) DOT), 1, YprocR,
                               YmyprocD );
                  else
                     Csgesd2d( ctxt, 1, 1, ((char *) DOT), 1, YmyprocD,
                               YprocR );
               }
               else if( YmyprocR == YprocR )
               {
                  if( XisRow )
                     Csgerv2d( ctxt, 1, 1, ((char *) DOT), 1, XprocR,
                               XmyprocD );
                  else
                     Csgerv2d( ctxt, 1, 1, ((char *) DOT), 1, XmyprocD,
                               XprocR );
               }
            }
         }
            ...
            if( YmyprocR == YprocR )
            {
               if( YisRow )
               {
                  ...
                  if( YmyprocD == XprocR )
                     ...
                     Csgebr2d( ctxt, ROW,    top, 1, 1, ((char*)DOT), 1,
                               YprocR, XprocR );
               }
               else
               {
                  ...
                  if( YmyprocD == XprocR )
                     ...
                     Csgebr2d( ctxt, COLUMN, top, 1, 1, ((char*)DOT), 1,
                               XprocR, YprocR );
               }
            }
         }
