...
*  =======
*
*  PB_Ctzatrmv  performs the matrix-vector  operation
*
...
*     or
*
*     y := abs( alpha )*abs( conjg( A' ) )*abs( x ) + abs( y ),
*
*  where alpha is a real scalar, y is a real vector, x is a vector and A
*  is an m by n trapezoidal triangular matrix.
*
*  Arguments
*  =========
*
...
*
*             TRANS = 'C' or 'c':
*                y := abs( alpha )*abs( A' )*abs( x ) + abs( y ) or
*                y := abs( alpha )*abs( conjg(A') )*abs( x ) + abs( y ).
*
...
*/
   ...
   if( ( Mupcase( UPLO[0] ) == CLOWER ) || ( Mupcase( UPLO[0] ) == CUPPER ) )
   {
      ...
      {
         TYPE->Fagemv( C2F_CHAR( TRANS ), &M, &N, ALPHA, Aptr, &M, X, &LDX,
                       TYPE->one, Y, &ione );
      }
      else
      {
         TYPE->Fagemv( C2F_CHAR( TRANS ), &M, &N, ALPHA, Aptr, &M, X, &ione,
                       TYPE->one, Y, &LDY );
      }
      if( Aptr ) free( Aptr );
   }
   else
   {
      if( Mupcase( TRANS[0] ) == CNOTRAN )
      {
         TYPE->Fagemv( C2F_CHAR( TRANS ), &M, &N, ALPHA, A, &LDA, X, &LDX,
                       TYPE->one, Y, &ione );
      }
      else
      {
         TYPE->Fagemv( C2F_CHAR( TRANS ), &M, &N, ALPHA, A, &LDA, X, &ione,
                       TYPE->one, Y, &LDY );
      }
   }
/*
