...
*  =======
*
*  PZDOTC  forms the dot product of two subvectors,
*
*     DOT := sub( X )**H * sub( Y ),
*
*  where
...
*/
   ...
   {
      PB_Cchkvec( ctxt, "PZDOTC", "X", *N, 1, Xi, Xj, Xd, *INCX,  6, &info );
      PB_Cchkvec( ctxt, "PZDOTC", "Y", *N, 1, Yi, Yj, Yd, *INCY, 11, &info );
   }
   if( info ) { PB_Cabort( ctxt, "PZDOTC", info ); return; }
#endif
   DOT[REAL_PART] = ZERO;
   DOT[IMAG_PART] = ZERO;
/*
...
*/
      type = PB_Cztypeset(); size = type->size; dot = type->Fvvdotc;
/*
