 ...
 */
{
   ...
   void BI_cvvamx(MpiInt, char *, char *);
   void BI_cvvamx2(MpiInt, char *, char *);
 ...
 */
   ...
   if (!ScaLAPACK_Index64Mul((ScaLAPACK_Index64) Mpval(m),
      ...
      BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                  "Complex max span overflow (m=%d, n=%d)",
                  Mpval(m), Mpval(n));
/*
 ...
 */
   if (Mpval(ldia) != -1)
   {
      ...
      if (!ScaLAPACK_SizeTMul(count, sizeof(SCOMPLEX), &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Complex max buffer span overflow");
/*
 ...
 */
      ...
      if (!ScaLAPACK_SizeTAlignUp(pair_bytes, align_bytes, &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Complex max temporary buffer overflow");
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=_MPI_Op_create(BI_cMPI_amx2, i, &BlacComb);
      }
      else
      {
         ierr=_MPI_Op_create(BI_cMPI_amx, i, &BlacComb);
