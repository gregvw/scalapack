 ...
 */
{
   ...
   void BI_dvvsum(MpiInt, char *, char *);
/*
 ...
 */
   ...
   if (!ScaLAPACK_Index64Mul((ScaLAPACK_Index64) Mpval(m),
       ...
       !ScaLAPACK_SizeTMul(count, sizeof(double), &pair_bytes))
      BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                  "Double sum span overflow (m=%d, n=%d)",
                  Mpval(m), Mpval(n));
/*
 ...
 */
   if ( (Mpval(m) == tlda) || (Mpval(n) == 1) )
   {
      ...
      if (!ScaLAPACK_SizeTToApiInt(pair_bytes, &request_bytes))
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Double sum buffer span overflow");
 ...
 */
   else
   {
      if (!ScaLAPACK_SizeTMul((size_t) 2, pair_bytes, &total_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Double sum temporary buffer overflow");
