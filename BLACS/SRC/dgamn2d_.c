 ...
 */
{
   ...
   void BI_dvvamn(MpiInt, char *, char *);
   void BI_dvvamn2(MpiInt, char *, char *);
 ...
 */
   ...
   if (!ScaLAPACK_Index64Mul((ScaLAPACK_Index64) Mpval(m),
      ...
      BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                  "Double min span overflow (m=%d, n=%d)",
                  Mpval(m), Mpval(n));
/*
 ...
 */
   ...
   {
      vvop = BI_dvvamn;
      if (!ScaLAPACK_SizeTMul(count, sizeof(double), &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Double min buffer span overflow");
/*
 *    For performance, insist second buffer is at least 8-byte aligned
 */
      align_bytes = (sizeof(double) > 8) ? sizeof(double) : (size_t) 8;
      if (!ScaLAPACK_SizeTAlignUp(pair_bytes, align_bytes, &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Double min temporary buffer overflow");
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=_MPI_Op_create(BI_dMPI_amn2, i, &BlacComb);
      }
      else
      {
         ierr=_MPI_Op_create(BI_dMPI_amn, i, &BlacComb);
