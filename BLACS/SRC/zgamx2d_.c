 ...
 */
{
   ...
   void BI_zvvamx(MpiInt, char *, char *);
   void BI_zvvamx2(MpiInt, char *, char *);
 ...
 */
   ...
   if (!ScaLAPACK_Index64Mul((ScaLAPACK_Index64) Mpval(m),
      ...
      BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                  "Double complex max span overflow (m=%d, n=%d)",
                  Mpval(m), Mpval(n));
/*
 ...
 */
   if (Mpval(ldia) != -1)
   {
      ...
      if (!ScaLAPACK_SizeTMul(count, sizeof(DCOMPLEX), &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Double complex max buffer span overflow");
/*
 ...
 */
      ...
      if (!ScaLAPACK_SizeTAlignUp(pair_bytes, align_bytes, &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Double complex max temporary buffer overflow");
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=_MPI_Op_create(BI_zMPI_amx2, i, &BlacComb);
      }
      else
      {
         ierr=_MPI_Op_create(BI_zMPI_amx, i, &BlacComb);
