 ...
 */
{
   ...
   void BI_svvamn(MpiInt, char *, char *);
   void BI_svvamn2(MpiInt, char *, char *);
 ...
 */
   ...
   if (!ScaLAPACK_Index64Mul((ScaLAPACK_Index64) Mpval(m),
      ...
      BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                  "Real min span overflow (m=%d, n=%d)",
                  Mpval(m), Mpval(n));
/*
 ...
 */
      ...
      if (!ScaLAPACK_SizeTAlignUp(pair_bytes, align_bytes, &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Real min temporary buffer overflow");
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=_MPI_Op_create(BI_sMPI_amn2, i, &BlacComb);
      }
      else
      {
         ierr=_MPI_Op_create(BI_sMPI_amn, i, &BlacComb);
