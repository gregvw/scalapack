 ...
 */
   ...
   if (!ScaLAPACK_Index64Mul((ScaLAPACK_Index64) Mpval(m),
      ...
      BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                  "Integer combine span overflow (m=%d, n=%d)",
                  Mpval(m), Mpval(n));
/*
 ...
 */
   ...
   {
      vvop = BI_ivvamx;
      if (!ScaLAPACK_SizeTMul(count, sizeof(Int), &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Integer combine buffer span overflow");
/*
 *    For performance, insist second buffer is at least 8-byte aligned
 */
      align_bytes = (sizeof(Int) > 8) ? sizeof(Int) : (size_t) 8;
      if (!ScaLAPACK_SizeTAlignUp(pair_bytes, align_bytes, &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Integer combine temporary buffer overflow");
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=_MPI_Op_create(BI_iMPI_amx2, i, &BlacComb);
      }
      else
      {
         ierr=_MPI_Op_create(BI_iMPI_amx, i, &BlacComb);
