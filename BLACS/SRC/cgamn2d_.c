 ...
 */
{
   ...
   void BI_cvvamn(MpiInt, char *, char *);
   void BI_cvvamn2(MpiInt, char *, char *);
 ...
 */
   ...
   if (!ScaLAPACK_Index64Mul((ScaLAPACK_Index64) Mpval(m),
      ...
      BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                  "Complex min span overflow (m=%d, n=%d)",
                  Mpval(m), Mpval(n));
/*
 ...
 */
   ...
   {
      vvop = BI_cvvamn;
      if (!ScaLAPACK_SizeTMul(count, sizeof(SCOMPLEX), &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Complex min buffer span overflow");
/*
 *    For performance, insist second buffer is at least 8-byte aligned
 */
      align_bytes = (sizeof(SCOMPLEX) > 8) ? sizeof(SCOMPLEX) : (size_t) 8;
      if (!ScaLAPACK_SizeTAlignUp(pair_bytes, align_bytes, &pair_bytes) ||
         ...
         BI_BlacsErr(Mpval(ConTxt), __LINE__, __FILE__,
                     "Complex min temporary buffer overflow");
 ...
 */
   ...
   case ' ':         /* use MPI's reduction by default */
      ...
      if (Mpval(ldia) == -1)
      {
         ierr=_MPI_Op_create(BI_cMPI_amn2, i, &BlacComb);
      }
      else
      {
         ierr=_MPI_Op_create(BI_cMPI_amn, i, &BlacComb);
