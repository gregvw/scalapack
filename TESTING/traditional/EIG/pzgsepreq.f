*
*
      SUBROUTINE PZGSEPREQ( NIN, MEM, MEMSIZE, NOUT, ISEED, NTESTS,
...
*  =======
*
*  PZGSEPREQ performs one request from the input file 'SEP.dat'
*  A request is the cross product of the specifications in the
*  input file.  PZGSEPREQ prints one line per test.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                   DESCINIT, PDSEPINFO, PZGSEPTST, PZLASIZEGSEP
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*
     ...
     $                                    CONTEXT, LDA, INFO )
                           CALL PZLASIZEGSEP( DESCA, IPREPAD, IPOSTPAD,
...
*
     ...
     $                               IPREPAD + 1 )* ( ZPLXSZ / DBLESZ )
                           NTESTS = NTESTS + 1
                           IF( LLRWORK.LT.RSIZETST ) THEN
                              NSKIPPED = NSKIPPED + 1
                           ELSE
                              CALL PZGSEPTST( DESCA, UPLOS( UPLO ), N,
     $                                        MATTYPES( MATTYPE ),
