      SUBROUTINE PSELGET_I8( SCOPE, TOP, ALPHA, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*
      EXTERNAL           BLACS_GRIDINFO, SGEBR2D, SGEBS2D, INFOG2L_I8
      LOGICAL            LSAME
      EXTERNAL           LSAME
      INTRINSIC          INT
*
      ...
      IF( LSAME( SCOPE, 'R' ) ) THEN
         IF( MYROW.EQ.IAROW ) THEN
            IF( MYCOL.EQ.IACOL ) THEN
               ...
               CALL SGEBS2D( ICTXT, SCOPE, TOP, 1, 1, A( IOFFA ), 1 )
               ALPHA = A( IOFFA )
            ELSE
               CALL SGEBR2D( ICTXT, SCOPE, TOP, 1, 1, ALPHA, 1,
     $                       IAROW, IACOL )
            END IF
         END IF
