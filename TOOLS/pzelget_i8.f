      SUBROUTINE PZELGET_I8( SCOPE, TOP, ALPHA, A, IA, JA, DESCA )
      IMPLICIT NONE
*
...
*
      EXTERNAL           BLACS_GRIDINFO, ZGEBR2D, ZGEBS2D, INFOG2L_I8
      LOGICAL            LSAME
      EXTERNAL           LSAME
      INTRINSIC          INT
*
      ...
      IF( LSAME( SCOPE, 'R' ) ) THEN
         IF( MYROW.EQ.IAROW ) THEN
            IF( MYCOL.EQ.IACOL ) THEN
               ...
               CALL ZGEBS2D( ICTXT, SCOPE, TOP, 1, 1, A( IOFFA ), 1 )
               ALPHA = A( IOFFA )
            ELSE
               CALL ZGEBR2D( ICTXT, SCOPE, TOP, 1, 1, ALPHA, 1,
     $                       IAROW, IACOL )
            END IF
         END IF
